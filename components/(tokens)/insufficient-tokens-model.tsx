...
// =============================================================================
// isaacpaha.com — Insufficient Tokens Modal (Mobile Optimized)
// Fully responsive design with device-specific layouts
// =============================================================================

import React, { useState, useEffect, useRef } from "react";
import { motion, AnimatePresence } from "framer-motion";
import { TOKEN_PACKAGES } from "@/lib/tokens/token-packages";

// ─── Package Card (Mobile Optimized) ─────────────────────────────────────────
...
function PackageCard({
  ...
  return (
    ...
    >
      {pkg.badge && (
        <div
          className="absolute top-0 right-0 text-[8px] sm:text-[9px] font-black uppercase tracking-widest px-2 py-1 rounded-bl-sm"
          style={{
      ...
      <div className="px-3 sm:px-4 py-3 sm:py-4">
        <div className="flex items-center justify-between mb-1">
          <p className="text-xs sm:text-sm font-black text-white">{pkg.label}</p>
        ...
        <div className="flex items-center justify-between">
          <p className="text-base sm:text-xl font-black text-white">
            {pkg.tokens.toLocaleString()} <span className="text-amber-400 text-sm sm:text-base">🪙</span>
          </p>
          <p className="text-[8px] sm:text-[10px] text-white/30 font-mono">{pkg.per1k}</p>
        </div>
      </div>
...
function CoinCounter({ value, color }: { value: number; color: string }) {
  ...
  return (
    <span style={{ color }} className="font-black tabular-nums text-base sm:text-lg">
      {display.toLocaleString()}
    </span>
  );
}

// ─── Mobile Game Card (Optimized for small screens) ─────────────────────────

function MobileGameCard({ game }: { game: any }) {
  return (
    <div
      className="rounded-sm p-2.5 sm:p-3"
      style={{ backgroundColor: "rgba(255,255,255,0.03)", border: "1px solid rgba(255,255,255,0.07)" }}
    >
      <div className="text-xl sm:text-2xl mb-0.5">{game.emoji}</div>
      <p className="text-xs font-bold text-white">{game.name}</p>
      <p className="text-[9px] sm:text-[10px] text-white/40 mt-0.5 line-clamp-1">{game.desc}</p>
      <div className="flex items-center gap-1.5 mt-1.5">
        <span className="text-[9px] sm:text-[10px] font-bold text-amber-400">{game.reward}</span>
        <span className="text-[8px] text-white/25">·</span>
        <span className="text-[9px] sm:text-[10px] text-white/30">{game.time}</span>
      </div>
    </div>
  );
}
...
export function InsufficientTokensModal({
  ...
  const games = [
    ...
    { emoji: "⚡", name: "Reaction", reward: "6–9 🪙", time: "30s", desc: "Test reflexes" },
  ];
      ...
      {open && (
        <>
          ...
          <motion.div
            ...
            className="fixed inset-x-0 bottom-0 sm:inset-auto sm:left-1/2 sm:top-1/2 sm:-translate-x-1/2 sm:-translate-y-1/2 z-[9999] flex items-end sm:items-center justify-center p-0 sm:p-4 pointer-events-none"
          >
            <div
              className="pointer-events-auto w-full sm:w-auto sm:max-w-md rounded-t-2xl sm:rounded-sm overflow-hidden shadow-2xl"
              style={{
            ...
            >
              ...
              <div
                className="relative px-4 sm:px-6 pt-6 sm:pt-8 pb-4 sm:pb-6"
                style={{
              ...
              >
                <button
                  onClick={onClose}
                  className="absolute top-3 right-3 sm:top-4 sm:right-4 w-7 h-7 sm:w-8 sm:h-8 flex items-center justify-center rounded-sm text-white/30 hover:text-white/70 hover:bg-white/5 transition-colors text-sm"
                >
                  ✕
                </button>

                {/* Coin animation (smaller on mobile) */}
                ...
                <h2 className="text-lg sm:text-xl font-black text-white text-center mb-2">
                  Not enough tokens
                </h2>

                <div className="flex items-center justify-center gap-3 sm:gap-4 mb-3 sm:mb-4">
                  <div className="text-center">
                    <p className="text-[8px] sm:text-[10px] text-white/30 uppercase tracking-widest font-bold mb-0.5">You have</p>
                    <CoinCounter value={balance} color="#f59e0b" />
                    <span className="text-amber-400 text-base sm:text-lg ml-0.5">🪙</span>
                  </div>
                  <div className="w-px h-6 sm:h-8 bg-white/10" />
                  <div className="text-center">
                    <p className="text-[8px] sm:text-[10px] text-white/30 uppercase tracking-widest font-bold mb-0.5">
                      {toolName ? `${toolName} needs` : "Tool needs"}
                    </p>
                    <CoinCounter value={required} color="rgb(248,113,113)" />
                    <span className="text-red-400 text-base sm:text-lg ml-0.5">🪙</span>
                  </div>
                </div>
                ...
                <motion.div
                  ...
                  className="text-center text-xs sm:text-sm text-white/50 leading-snug"
                >
                  You need{" "}
              ...
              <div className="flex border-b border-white/8 px-4 sm:px-6">
                {([
                  { id: "game", label: "🎮 Earn", sub: "Play a game" },
                  { id: "buy", label: "⚡ Buy", sub: "Instant top-up" },
                ] as const).map((t) => (
                  <button
                    key={t.id}
                    onClick={() => setTab(t.id)}
                    className="flex-1 py-2.5 sm:py-3 text-center transition-colors relative"
                  >
                    <p className={`text-xs sm:text-sm font-bold transition-colors ${tab === t.id ? "text-white" : "text-white/40 hover:text-white/60"}`}>
                      {t.label}
                    </p>
                    <p className="text-[8px] sm:text-[10px] text-white/25 mt-0.5">{t.sub}</p>
                    <motion.div
              ...
              <div className="px-4 sm:px-6 py-4 sm:py-5 max-h-[50vh] sm:max-h-none overflow-y-auto">
                <AnimatePresence mode="wait">
                  ...
                  {tab === "game" && (
                    ...
                    >
                      ...
                      >
                        <p className="text-xs sm:text-sm font-bold text-amber-300 mb-1">How it works</p>
                      ...
                      <div className="grid grid-cols-2 gap-2 sm:gap-3">
                        {games.map((game) => (
                          <MobileGameCard key={game.name} game={game} />
                        ))}
                      </div>
                      ...
                      <motion.button
                        ...
                        whileTap={{ scale: 0.98 }}
                        className="w-full py-3 sm:py-4 rounded-sm font-black text-xs sm:text-sm transition-colors"
                        style={{ backgroundColor: "#f59e0b", color: "#0a0a0c" }}
                      >
                        Open Game Hub → Earn Free Tokens
                      </motion.button>
                      <p className="text-center text-[8px] sm:text-[10px] text-white/25">
                  ...
                  {tab === "buy" && (
                    ...
                    >
                      ...
                      <div
                        className="flex items-center justify-between px-3 sm:px-4 py-2.5 sm:py-3 rounded-sm"
                        style={{ backgroundColor: "rgba(255,255,255,0.04)", border: "1px solid rgba(255,255,255,0.08)" }}
                      >
                        <div>
                          <p className="text-[10px] sm:text-xs text-white/40">You're buying</p>
                          <p className="text-sm sm:text-base font-black text-amber-400">
                            {selectedPackage.tokens.toLocaleString()} tokens
                          </p>
                          <p className="text-[8px] sm:text-[10px] text-white/25 mt-0.5">
                        ...
                        <div className="text-right">
                          ...
                          <p className="text-[8px] sm:text-[10px] text-white/30">{selectedPackage.per1k}</p>
                        </div>
                      </div>

                      {checkoutError && (
                        <p className="text-[10px] sm:text-xs text-red-400 text-center">{checkoutError}</p>
                      )}
                      ...
                      <motion.button
                        ...
                        whileHover={{ scale: 1.01 }}
                        whileTap={{ scale: 0.98 }}
                        className="w-full py-3 sm:py-4 rounded-sm font-black text-xs sm:text-sm flex items-center justify-center gap-2 disabled:opacity-60 transition-all"
                        style={{ backgroundColor: "#f59e0b", color: "#0a0a0c" }}
                      >
                        {loadingCheckout ? (
                          <>
                      ...
                      <div className="flex items-center justify-center gap-2 sm:gap-4 flex-wrap">
                        {[
                          ...
                          <div key={s.label} className="flex items-center gap-1">
                            ...
                            <span className="text-[8px] sm:text-[10px] text-white/30 font-semibold whitespace-nowrap">{s.label}</span>
                          </div>
                        ))}
                      </div>
                    </motion.div>
              ...
              >
                <p className="text-[8px] sm:text-[10px] text-white/20">
                  Tokens power AI generation
                </p>
                <button
                  onClick={onClose}
                  className="text-[8px] sm:text-[10px] text-white/30 hover:text-white/60 transition-colors"
                >
                  Maybe later
                </button>
              </div>
...
// //   />
// // FIX: TOKEN_PACKAGES is now imported from "@/lib/tokens/token-packages"
// //      instead of from the API route. This breaks the chain that was pulling
// //      prismadb → mariadb → tls (Node.js built-in) into the browser bundle.
// // =============================================================================
 
// import React, { useState, useEffect, useRef } from "react";
// import { motion, AnimatePresence }             from "framer-motion";
// import { TOKEN_PACKAGES }                      from "@/lib/tokens/token-packages"; // ← FIXED

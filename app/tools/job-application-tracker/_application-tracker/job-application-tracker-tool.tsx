...
// function AICoach({ apps, isSignedIn, onInsufficientTokens }: { apps: JobApplication[]; isSignedIn: boolean; onInsufficientTokens: (info: TokenGateInfo) => void }) {
//   const [mode,    setMode]    = useState<"insights"|"interview"|"motivation"|"next-steps">("insights");
//   const [role,    setRole]    = useState("");
//   const [loading, setLoading] = useState(false);
//   const [result,  setResult]  = useState("");
...
//           required: data.requiredTokens ?? 0,
//           balance: data.tokenBalance ?? 0,
//           toolName: data.toolName ?? "Job Application Tracker",
//         });
//         setLoading(false);
//         return;  // stop here - modal is shown by parent
//       }
...
//       )}
//     </div> 
//   );         // ← This was also missing (function closing brace)
// }


// // ─── MAIN COMPONENT ───────────────────────────────────────────────────────────

// export function JobApplicationTrackerTool({ onInsufficientTokens }: { onInsufficientTokens: (info: TokenGateInfo) => void }) {
//   const { isSignedIn, user } = useUser();
//   const [tab,          setTab]          = useState<"tracker"|"stats"|"ai"|"ladder"|"community"|"share">("tracker");
//   const [view,         setView]         = useState<"list"|"kanban">("list");
//   const [apps,         setApps]         = useState<JobApplication[]>([]);

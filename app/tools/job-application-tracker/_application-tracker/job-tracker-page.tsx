...
// app/tools/job-application-tracker/_application-tracker/job-tracker-page.tsx
//
// Changes:
//  - No longer imports from @/lib/data/tools-data
//  - Receives `tool` and `relatedTools` as props (NormalisedTool from DB)
//  - Uses <ToolViewTracker> for DB view count
//  - Uses <ToolReviews> for live DB reviews + submit form
//  - ToolCard receives NormalisedTool shape
// =============================================================================
...
export function JobTrackerPage({ initialProfile, tool, relatedTools }: JobTrackerPageProps) {
  ...
  // Derive category display from the DB category string
  ...
  const TABS = [
    ...
    ...(isSignedIn ? [{ id: "dashboard", label: "My Dashboard", icon: TrendingUp } as const] : []),
    ...
    <div className="min-h-screen bg-white text-gray-900">
      ...
      <div className="relative z-10 max-w-6xl mx-auto px-4 pt-24 pb-20">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 items-start mb-10">
              ...
              <div>
                <div className="flex flex-wrap items-center gap-2 mb-1">
                  ...
                  <span className="flex items-center gap-1.5 text-xs font-semibold px-2.5 py-1 rounded-xs border"
                    ...
                    <span className="w-1.5 h-1.5 rounded-full" style={{ backgroundColor: tool.accentColor }} />
                    {tool.status === "BETA" ? "Beta" : tool.status === "LIVE" ? "Live" : "Coming Soon"}
                  </span>
                  {tool.isNew && (
          ...
          {/* Right: profile card or sign-in prompt */}
          <div className="hidden lg:block">
            {isSignedIn && initialProfile ? (
              <div className="rounded-xs p-5 border"
                ...
                <div className="grid grid-cols-3 gap-2 mb-4">
                  {[
                    ...
                    <div key={s.label} className="text-center bg-white rounded-xs px-2 py-2 border border-gray-100">
                      <p className="text-lg font-black" style={{ color: tool.accentColor }}>{s.value}</p>
                      <p className="text-[10px] text-stone-400 font-semibold">{s.label}</p>
                    </div>
                  ))}
                </div>
                <button onClick={() => setActiveTab("dashboard")}
                  className="w-full text-sm font-bold py-2.5 rounded-xs transition-colors border"
                  style={{ color: tool.accentColor, borderColor: `${tool.accentColor}40` }}>
                  Open My Dashboard →
                </button>
              </div>
            ) : !isSignedIn ? (
              <div className="bg-gray-50 border border-gray-100 rounded-xs p-5">
                ...
                <Link href="/sign-in?redirect_url=/tools/job-application-tracker"
                  className="w-full flex items-center justify-center gap-2 text-sm font-bold text-white py-2.5 rounded-xs transition-colors"
                  style={{ backgroundColor: tool.accentColor }}>
                  Sign in — it's free
                </Link>
              </div>
          ...
          {TABS.map((t) => (
            <button key={t.id} onClick={() => setActiveTab(t.id as typeof activeTab)}
              className={`flex items-center gap-2 px-5 py-3 text-sm font-semibold border-b-2 transition-all whitespace-nowrap ${
                activeTab === t.id ? "border-current text-current" : "border-transparent text-gray-400 hover:text-gray-700"
              }`}
              ...
              <div className="rounded-xs p-5 border"
                ...
                {[
                  ...
                  <div key={s.text} className="flex items-start gap-2.5 mb-3 last:mb-0">
                    <s.icon className="w-4 h-4 mt-0.5 flex-shrink-0" style={{ color: tool.accentColor }} />

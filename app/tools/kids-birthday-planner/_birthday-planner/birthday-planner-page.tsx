...
function PartyListDashboard({ onReopenParty, accentColor }: {
          ...
          <motion.div key={p.id} layout initial={{ opacity: 0, y: 3 }} animate={{ opacity: 1, y: 0 }}
            ...
            <div className="p-4">
              <div className="flex items-start gap-3 mb-3">
                <div className="w-10 h-10 rounded-sm flex items-center justify-center text-xl flex-shrink-0" style={{ backgroundColor: `${accentColor}12` }}>🎂</div>
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-2 flex-wrap mb-0.5">
                    ...
                    <span className="text-[10px] font-bold px-2 py-0.5 rounded-sm" style={{ color: accentColor, backgroundColor: `${accentColor}15` }}>{themeName}</span>
                  </div>
...
// ─── Feature list for guide tab (remains static as it's UI content) ─────────
...
interface BirthdayPlannerPageProps {
  isSignedIn:   boolean;
...
export function BirthdayPlannerPage({ isSignedIn, tool, relatedTools }: BirthdayPlannerPageProps) {
  ...
  const catCfg = CATEGORY_CFG[tool.category] ?? { icon: "🎂", color: "#f43f5e" };
      ...
      <div className="relative z-10 max-w-6xl mx-auto px-4 pt-24 pb-20">
            ...
            <div className="flex flex-wrap items-center gap-5 text-sm text-gray-500 mb-5">
              {tool.usageCount > 0 && (
                <span className="flex items-center gap-1.5">
                  ...
                  {tool.usageCount.toLocaleString()} parties planned
                </span>
              )}
              {tool.ratingCount > 0 && (
                <span className="flex items-center gap-1.5">
                ...
                {!isSignedIn && (
                  <div className="flex items-start gap-3 mt-5 bg-rose-50 border border-rose-100 rounded-xs px-4 py-3.5">
                    ...
                    <p className="text-xs text-rose-700 leading-relaxed">
                      ...
                      <Link href="/sign-in?redirect_url=/tools/kids-birthday-planner" className="underline">Sign in →</Link>
                    </p>
                  </div>
                )}
              </motion.div>
              ...
              <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }}>
                {isSignedIn ? (
                  ...
                  <div className="flex flex-col items-center justify-center py-20 text-center bg-gray-50 border border-gray-100 rounded-xs">
                    ...
                    <Link href="/sign-in?redirect_url=/tools/kids-birthday-planner"
        ...
        {relatedTools.length > 0 && (
          <div className="mt-20 pt-12 border-t border-gray-100">
            <h2 className="text-2xl font-black text-gray-900 mb-8">More {tool.category} Tools</h2>

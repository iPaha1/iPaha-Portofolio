...
function ReadingProgress({ accentColor }: { accentColor: string }) {
  ...
  return <div className="fixed top-0 left-0 right-0 z-50 h-0.5"><div className="h-full transition-[width] duration-100" style={{ backgroundColor: accentColor, width: `${w}%` }} /></div>;
}
...
interface PhysicsEnginePageProps {
  isSignedIn:   boolean;
...
export function PhysicsEnginePage({ isSignedIn, tool, relatedTools }: PhysicsEnginePageProps) {
  ...
  const catCfg = CATEGORY_CFG[tool.category] ?? { icon: "⚛️", color: "#0ea5e9" };
                ...
                {isSignedIn
                  ...
                  : (
                    <div className="flex flex-col items-center justify-center py-20 text-center bg-gray-50 border border-gray-100 rounded-xs">
                      ...
                      <Link href="/sign-in?redirect_url=/tools/physics-engine"

...
interface MessageRewriterPageProps {
  isSignedIn:   boolean;
...
export function MessageRewriterPage({ isSignedIn, tool, relatedTools }: MessageRewriterPageProps) {
  ...
  const catCfg = CATEGORY_CFG[tool.category] ?? { icon: "✍️", color: "#e11d48" };
      ...
      <div className="bg-white border-b border-stone-100">
        <div className="max-w-7xl mx-auto px-4 py-8 md:py-12">
          ...
          <div className="flex items-center gap-2 text-xs text-stone-400 mb-6">
            ...
            <span className="text-stone-600 font-semibold">{tool.name}</span>
          </div>
          ...
          <div className="flex items-start justify-between gap-6 flex-wrap">
            <div className="flex-1 min-w-0">
              ...
              <div className="flex items-center gap-2 mb-3 flex-wrap">
                ...
                {tool.tokenCost ? (
                  <span className="text-[10px] font-bold text-amber-600 bg-amber-50 border border-amber-100 px-2 py-1 rounded-xs">
              ...
              <div className="flex items-center gap-5 mt-5 flex-wrap">
                {tool.ratingCount > 0 && (
                  <div className="flex items-center gap-1">
                    {[1,2,3,4,5].map(i => (
                      <Star key={i} className={`w-3.5 h-3.5 ${i <= Math.round(tool.ratingAvg) ? "fill-amber-400 text-amber-400" : "text-gray-200"}`} />
                    ))}
                    <span className="text-xs font-bold text-stone-600 ml-1">{tool.ratingAvg.toFixed(1)}</span>
                    <span className="text-xs text-stone-400 ml-0.5">({tool.ratingCount.toLocaleString()})</span>
                  </div>
                )}
                {tool.usageCount > 0 && (
                  <div className="flex items-center gap-1.5 text-xs text-stone-400">
                    <Users className="w-3.5 h-3.5" />{tool.usageCount.toLocaleString()} uses this month
                  </div>
                )}
                {tool.buildTime && (
                  <div className="flex items-center gap-1.5 text-xs text-stone-400">
                    <Clock className="w-3.5 h-3.5" />{tool.buildTime}
                  </div>
                )}
              </div>
              ...
              {tool.features.length > 0 && (
                <div className="flex flex-wrap gap-2 mt-4">
                  {tool.features.slice(0, 4).map((f) => (
                    <span key={f} className="inline-flex items-center gap-1.5 text-xs text-gray-600 bg-gray-50 border border-gray-100 px-2 py-1 rounded-xs">
            ...
            <div className="hidden lg:block">
              <div className="border border-stone-100 rounded-xs p-5 w-64" style={{ backgroundColor: `${ACCENT}05` }}>
            ...
            {[
              ...
              <button key={id} onClick={() => setActiveTab(id as any)}
                className={`flex items-center gap-2 text-xs font-bold px-5 py-4 border-b-2 transition-all whitespace-nowrap ${
                  activeTab === id
                    ? "border-current text-rose-600"
                    : "border-transparent text-stone-400 hover:text-stone-700"
                }`}
                style={activeTab === id ? { borderColor: ACCENT, color: ACCENT } : {}}>
      ...
      <div className="max-w-7xl mx-auto px-4 py-8">
        <div className="flex gap-8">
          ...
          <div className="hidden xl:block w-72 flex-shrink-0 space-y-5">
            ...
            {/* Insight card */}
            <div className="rounded-xs p-5 border" style={{ backgroundColor: `${ACCENT}08`, borderColor: `${ACCENT}20` }}>
              <div className="flex items-start gap-2.5 mb-3">
                <Lightbulb className="w-4 h-4 flex-shrink-0 mt-0.5" style={{ color: ACCENT }} />
                <p className="text-xs font-black" style={{ color: ACCENT }}>The real problem with most messages</p>
              </div>
              ...
              <p className="text-xs font-semibold mt-2 leading-relaxed" style={{ color: ACCENT }}>
            ...
            {relatedTools.length > 0 && (
              <div>
                <p className="text-[10px] font-black text-stone-400 uppercase tracking-wider mb-3">More {tool.category} Tools</p>
                <div className="space-y-2">
                  {relatedTools.map(t => (

...
// =============================================================================
// isaacpaha.com — Idea Detail Client
// app/ideas/_ideas/idea-detail-client.tsx
// =============================================================================
...
import {
  ArrowLeft, Eye, Heart, MessageSquare, Clock,
  Share2, BookOpen, ArrowRight, Check, ChevronUp, Loader2,
  CornerDownRight,
} from "lucide-react";
...
interface IdeaCommentData {
  ...
  isEdited:   boolean;
  replies:    Omit<IdeaCommentData, "replies">[];
}
...
const CommentRow = ({
  ...
  const COLORS = ["#f59e0b","#10b981","#8b5cf6","#3b82f6","#ec4899","#14b8a6"];
  const avatarColor = COLORS[
    Math.abs(comment.authorName.split("").reduce((h, c) => (h << 5) - h + c.charCodeAt(0), 0)) % COLORS.length
  ];
  const initials = comment.authorName.split(" ").map((w) => w[0]).slice(0, 2).join("").toUpperCase();
  ...
  const toggleLike = async () => {
    ...
    await fetch(`/api/ideas/${slug}/comments/${comment.id}/like`, { method: "POST" }).catch(() => {});
  };
  ...
  return (
    <motion.div
      ...
      className={`flex gap-3 ${depth > 0 ? "ml-10" : ""}`}
    >
      <div
      ...
      <div className="flex-1 min-w-0">
        <div className="bg-white/4 border border-white/8 rounded-xs p-4">
          <div className="flex items-center justify-between gap-2 mb-2">
            <p className="text-sm font-bold text-white">{comment.authorName}</p>
            <span className="text-xs text-white/25 shrink-0">{timeAgo(comment.createdAt)}</span>
          </div>
        ...
        <div className="flex items-center gap-3 mt-2 px-1">
          <button
            onClick={toggleLike}
            className={`flex items-center gap-1.5 text-xs transition-colors ${liked ? "text-amber-400" : "text-white/30 hover:text-white/60"}`}
          >
            <Heart className={`w-3.5 h-3.5 ${liked ? "fill-amber-400" : ""}`} />
        ...
        {comment.replies?.length > 0 && (
          <div className="mt-4 space-y-4">
            {comment.replies.map((r) => (
              <CommentRow key={r.id} comment={{ ...r, replies: [] }} slug={slug} depth={depth + 1} onReply={onReply} />
            ))}
          </div>
        )}
      </div>
...
const CommentsSection = ({ slug, count }: { slug: string; count: number }) => {
  ...
  const handleSubmit = async () => {
    ...
    setSubmitting(true);
    try {
      const res = await fetch(`/api/ideas/${slug}/comments`, {
        method:  "POST",
      ...
      if (!res.ok) {
        ...
        setName(""); setEmail(""); setComment(""); setReplyingTo(null);
        setSubmitted(true);
      }
    } catch {
  ...
  return (
    <div className="mt-16 pt-12 border-t border-white/8">
      ...
      <div className="bg-gray-900 border border-white/8 rounded-xs p-6 mb-10">
        {replyingTo && (
          <div className="flex items-center gap-2 text-xs text-amber-400 bg-amber-900/20 border border-amber-500/20 px-3 py-2 rounded-xs mb-4">
            <CornerDownRight className="w-3.5 h-3.5" />
            Replying to <span className="font-bold">{replyingTo.name}</span>
            <button onClick={() => setReplyingTo(null)} className="ml-auto text-amber-500 hover:text-amber-300 font-semibold">Cancel</button>
          </div>
        )}

        {submitted ? (
          <motion.div initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }}
            className="flex items-center gap-3 bg-green-900/20 border border-green-700/30 rounded-xs p-4"
          >
            <Check className="w-5 h-5 text-green-400" />
            <p className="text-sm text-green-300">Comment submitted — it will appear after moderation.</p>
          </motion.div>
        ) : (
          <>
            <p className="text-sm font-semibold text-white/60 mb-5">Share your thoughts on this idea</p>
            <div className="space-y-4">
              ...
              {error && (
                <p className="text-xs text-red-400 bg-red-900/20 border border-red-700/30 px-3 py-2 rounded-xs">{error}</p>
              )}
              <div className="flex items-center justify-between">
                <span className="text-xs text-white/20">{comment.length}/1000</span>
                <button onClick={handleSubmit} disabled={submitting || !name || !email || !comment}
                  className="flex items-center gap-2 bg-amber-500 hover:bg-amber-400 disabled:opacity-40 text-white text-sm font-semibold px-6 py-3 rounded-xs transition-all duration-200"
                >
...
export const IdeaDetailClient = ({ idea, related }: Props) => {
  ...
  // Initialise like state from DB on mount
  useEffect(() => {
    fetch(`/api/ideas/${idea.slug}/like`)
      .then((r) => r.json())
      .then((data) => {
  ...
  const toggleLike = async () => {
    ...
    try {
      const res = await fetch(`/api/ideas/${idea.slug}/like`, { method: "POST" });
  ...
  // Estimated reading time from content word count
  const readingTime = Math.max(1, Math.round(idea.content.split(/\s+/).length / 200));
      ...
      <div className="relative z-10 max-w-6xl mx-auto px-4 pt-28 pb-20">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-12">
          ...
          >
            ...
            >
              ...
              <div className="mt-10 bg-amber-900/15 border border-amber-500/15 rounded-xs p-6">
                ...
                <p className="text-white/50 text-sm leading-relaxed">
                  ...
                  <Link href="/ask-isaac" className="text-amber-400 hover:text-amber-300 underline">
                    ask me directly
                  </Link>
                  .
                </p>
            ...
            {/* Comments */}
            <CommentsSection slug={idea.slug} count={idea.commentCount} />
          </motion.article>
...
//             />
//             {error && (
//               <p className="text-xs text-red-400 bg-red-900/20 border border-red-700/30 px-3 py-2 rounded-xs">
//                 {error}
//               </p>
//             )}
//             <div className="flex items-center justify-between">
...
//                 disabled={submitting || !name || !email || !comment}
//                 className="flex items-center gap-2 bg-amber-500 hover:bg-amber-400 disabled:opacity-40 text-white text-sm font-semibold px-6 py-3 rounded-xs transition-all duration-200"
//               >
//                 {submitting && <Loader2 className="w-3.5 h-3.5 animate-spin" />}
//                 Post Comment

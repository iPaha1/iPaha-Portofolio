...
// =============================================================================
// isaacpaha.com — Ideas Sidebar
// app/ideas/_ideas/idea-side-bar.tsx
// =============================================================================
...
import type { DBIdea, IdeaCategory } from "@/lib/types/idea";
import { IDEA_CATEGORIES, IDEA_STATUS_CONFIG } from "@/lib/types/idea";
...
export const IdeasSidebar = ({ ideas, activeCategory, onCategory }: IdeasSidebarProps) => {
  // Trending: computed client-side from the passed list
  ...
  const catCounts = IDEA_CATEGORIES.map((cat) => ({
    ...cat,
    count: ideas.filter((i) => i.category === cat.key).length,
  })).sort((a, b) => b.count - a.count);

  // Status counts from live data
  const statusCounts = (Object.keys(IDEA_STATUS_CONFIG) as (keyof typeof IDEA_STATUS_CONFIG)[])
    .map((key) => ({
      key,
      ...IDEA_STATUS_CONFIG[key],
      ...
      >
        ...
        <div className="space-y-1">
          ...
          >
            ...
            <span className="text-xs bg-white/5 px-2 py-0.5 rounded-xs">{ideas.length}</span>
          </button>
              ...
              <div className="flex items-center gap-2">
                <div className="w-16 h-1 bg-white/5 rounded-full overflow-hidden">
                  <div
                    ...
                    style={{ width: `${(s.count / Math.max(ideas.length, 1)) * 100}%` }}
                  />
                </div>
      ...
      {trending.length > 0 && (
        ...
        >
          ...
          <ol className="space-y-4">
            {trending.map((idea, i) => {
              const score = idea.likeCount * 3 + idea.commentCount * 5 + idea.viewCount * 0.1;
              return (

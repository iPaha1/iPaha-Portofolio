...
// =============================================================================
// isaacpaha.com — Blog Comments Tab
// components/admin/blog/comments-tab.tsx
// =============================================================================
...
const STATUS_CFG: Record<CommentStatus, { label: string; color: string; bg: string; dot: string; icon: React.ElementType }> = {
...
function CommentRow({
  ...
  onModerate: (id: string, status: CommentStatus) => Promise<void>;
  ...
  const moderate = async (status: CommentStatus) => {
    setActing(true);
  ...
  return (
    <motion.div layout initial={{ opacity: 0, y: 3 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0 }}
      className={`group flex items-start gap-3 px-4 py-4 border-b border-stone-100 hover:bg-stone-50/60 transition-colors ${selected ? "bg-amber-50/30" : ""} ${comment.status === "PENDING" ? "border-l-2 border-l-amber-400" : ""}`}>

      {/* Checkbox */}
      <button onClick={() => onSelect(comment.id)} className="flex-shrink-0 mt-1">
        <div className={`w-4 h-4 rounded border-2 flex items-center justify-center transition-colors ${selected ? "bg-amber-500 border-amber-500" : "border-stone-200 group-hover:border-stone-400"}`}>
      ...
      <div className="flex-1 min-w-0">
        <div className="flex items-start gap-2 flex-wrap">
          ...
          <span className="text-[10px] font-semibold px-1.5 py-0.5 rounded-sm" style={{ color: sc.color, backgroundColor: sc.bg }}>
            {sc.label}
          </span>
        </div>
        ...
        <div className="flex items-center gap-2 mt-2">
          <a href={`/blog/${comment.post.slug}`} target="_blank" rel="noopener noreferrer"
            className="flex items-center gap-1 text-[11px] text-stone-400 hover:text-amber-600 transition-colors">
            <ExternalLink className="w-3 h-3" />
            On: &#34;{comment.post.title.slice(0, 50)}{comment.post.title.length > 50 ? "…" : ""}&#34;
          </a>
        </div>
      </div>
          ...
          <AnimatePresence>
            {menuOpen && (
              <motion.div initial={{ opacity: 0, scale: 0.95, y: -4 }} animate={{ opacity: 1, scale: 1, y: 0 }}
                ...
                className="absolute right-0 top-8 z-20 w-40 bg-white border border-stone-200 rounded-sm shadow-xl overflow-hidden">
                {comment.status !== "APPROVED" && (
                  <button onClick={() => moderate("APPROVED")}
...
export function CommentsTab() {
  ...
  const [stats,        setStats]        = useState({ pending: 0, approved: 0, spam: 0, rejected: 0 });

  const fetchComments = useCallback(async (opts?: { status?: string; search?: string; pg?: number }) => {
    setLoading(true);
  ...
  const bulkModerate = async (status: CommentStatus) => {
    const ids = [...selected];
    await Promise.all(ids.map((id) => fetch(`/api/admin/blog/comments/${id}`, {
      method: "PATCH", headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ status }),
    })));
    ...
    <div className="flex flex-col h-full overflow-hidden">
      ...
      <div className="grid grid-cols-4 gap-0 border-b border-stone-100 flex-shrink-0">
        {Object.entries(STATUS_CFG).map(([key, cfg]) => {
          ...
          const Icon = cfg.icon;
          return (
            <button key={key} onClick={() => { setStatusFilter(key as CommentStatus); fetchComments({ status: key }); }}
      ...
      <div className="flex items-center gap-2 px-4 py-3 border-b border-stone-100 flex-wrap flex-shrink-0">
        ...
        <div className="flex gap-1">
          {(["ALL", "PENDING", "APPROVED", "SPAM", "REJECTED"] as const).map((s) => (
            <button key={s} onClick={() => { setStatusFilter(s as CommentStatus | "ALL"); fetchComments({ status: s }); }}
              className={`text-[11px] font-semibold px-2.5 py-1.5 rounded-sm border transition-colors ${
                statusFilter === s ? "bg-amber-50 text-amber-700 border-amber-200" : "bg-white text-stone-500 border-stone-200 hover:border-stone-300"
              }`}>
        ...
        <button onClick={() => fetchComments()} className="text-stone-400 hover:text-stone-700 transition-colors ml-auto">
        ...
        {selected.size > 0 && (
          <div className="flex items-center gap-2 pl-2 border-l border-stone-200">
            ...
            <button onClick={() => bulkModerate("APPROVED")} className="text-[11px] font-bold text-emerald-700 border border-emerald-200 hover:bg-emerald-50 px-2 py-1.5 rounded-sm flex items-center gap-1 transition-colors">
              <Check className="w-3 h-3" />Approve all
            </button>
            <button onClick={() => bulkModerate("SPAM")} className="text-[11px] font-bold text-red-600 border border-red-200 hover:bg-red-50 px-2 py-1.5 rounded-sm flex items-center gap-1 transition-colors">
      ...
      <div className="flex-1 overflow-y-auto">
        {loading ? (
          ...
          <div className="flex flex-col items-center justify-center py-20 text-center">
            ...
            <p className="text-sm text-stone-400">
              {statusFilter === "PENDING" ? "No pending comments — all moderated!" : "No comments match your filter"}
            </p>
          </div>
        ) : (
          <AnimatePresence>
        ...
        {pages > 1 && (
          <div className="flex items-center justify-center gap-2 py-6">
            <button disabled={page <= 1} onClick={() => { setPage(page - 1); fetchComments({ pg: page - 1 }); }}
            ...
            <button disabled={page >= pages} onClick={() => { setPage(page + 1); fetchComments({ pg: page + 1 }); }}
              className="text-xs font-semibold text-stone-500 border border-stone-200 px-3 py-2 rounded-sm hover:border-amber-400 disabled:opacity-40 transition-colors">
              Next →
            </button>
          </div>
